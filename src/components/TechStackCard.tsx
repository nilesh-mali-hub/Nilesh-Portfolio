...
export function TechStackCard({ className = "", skills: propSkills }: TechStackCardProps) {
  const displaySkills = (propSkills && propSkills.length > 0) 
    ? propSkills.slice(0, 6).map((s, idx) => ({
        name: s.name || s.title || 'Skill',
        percentage: s.percentage ? Number(s.percentage) : [95, 92, 90, 88, 85, 80][idx % 6]
      }))
    : defaultSkills;
          ...
          {displaySkills.map((skill, index) => (
            <div key={index} className="flex flex-col gap-2">
              <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-neutral-400">
                ...
                <span className="font-mono text-[#D1FF52]">{skill.percentage}%</span>
              </div>
